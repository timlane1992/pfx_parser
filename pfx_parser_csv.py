...
def get_start_end_dates():
    """
    Prompt the user for start and end dates for data.
    :return: dict; keys are the strings 'start' and 'end',
             values are datetime objects
    """
...
def output_pfx_data(startdate, enddate):
    """
    Parse data from http://gd2.mlb.com/components/game/mlb/
    to get all pitchfx data from startdate until enddate.
    :param startdate: datetime object; represents the first date
                      for which pitchfx data is desired.
    :param startdate: datetime object; represents the first date
                      for which pitchfx data is desired.
    """
    data_dirname = "pfx_data"
    pitch_output_name = "pitch_table_{0}_{1}.csv".format(startdate.year,
                                                         startdate.month)
    atbat_output_name = "atbat_table_{0}_{1}.csv".format(startdate.year,
                                                         startdate.month)

    if not os.path.isdir(data_dirname) and not os.path.isfile(data_dirname):
        os.mkdir(data_dirname)
    elif os.path.isfile(data_dirname):
        print('Output directory name is already taken. '
              'Writing files to current working directory.')

    pitch_output_path = os.path.join(data_dirname, pitch_output_name)
    atbat_output_path = os.path.join(data_dirname, atbat_output_name)

    pitch_outfile=open(pitch_output_path, "a+", encoding='utf-8')
    atbat_outfile=open(atbat_output_path, "a+", encoding='utf-8')
    ...
    if os.stat(pitch_output_path).st_size==0:
    ...
    if os.stat(atbat_output_path).st_size==0:
    ...
    edited_pitch_path = '{0}.duplicates_removed'.format(pitch_output_path)
    edited_atbat_path = '{0}.duplicates_removed'.format(atbat_output_path)
    with open(pitch_output_path,'r') as in_file, open(edited_pitch_path,'w') as out_file:
        seen = set()
        for line in in_file:
            ...
            seen.add(line)
            out_file.write(line)
    with open(atbat_output_path,'r') as in_file, open(edited_atbat_path,'w') as out_file:
        seen = set()
        for line in in_file:
            if line in seen: continue
...
def run_interactive_mode():
    """
    Get the start and end dates from the user and then
    download and parse the corresponding pitchfx data.
    """
    timesd = get_start_end_dates()
    output_pfx_data(timesd['start'], timesd['end'])
